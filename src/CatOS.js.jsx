...
function Today({st,set}){
  ...
  return <div style={{display:"flex",flexDirection:"column",gap:16}}>
    ...
    <Card><SL icon="🌅" color={T.gold}>{isWE?"Weekend Morning":"Morning Routine"}</SL>
      {(isWE?[
        {k:"m1",l:"Let Luna out, feed Luna, start coffee ☕",s:"Morning"},
        {k:"m2",l:"Water + sunlight",s:"Morning"},
        {k:"m3",l:"Get ready",s:"No rush"},
      ]:[
        {k:"m1",l:"Let Luna out, feed Luna, start coffee ☕",s:"6:30 AM"},
        {k:"m2",l:"Water + sunlight",s:"6:30 AM"},
        {k:"m3",l:"Get ready",s:"6:45–8:05 AM"},
        {k:"m4",l:"Pack gym bag",s:"8:05 AM"},
        {k:"m5",l:"Grab gym bag + breakfast",s:"8:10 AM"},
        {k:"m6",l:"Out the door 🚗",s:"8:20 AM"},
      ]).map(i=><CB key={i.k} label={i.l} sub={i.s} color={T.gold} checked={!!st.morningChecks[i.k]} onChange={()=>tog("morningChecks",i.k)}/>)}
    </Card>

    {isWE?<Card><SL icon="🏠" color={T.gold}>Weekend Blocks</SL>
      {[
        {k:"we1",l:"Errands",s:"Groceries, returns, pickups"},
        {k:"we2",l:"Housework",s:"Laundry, cleaning, tidying"},
        {k:"we3",l:"Meal prep",s:"Cook & portion for the week"},
        {k:"we4",l:"Weekly planning",s:"Review calendar, set intentions"},
        {k:"we5",l:"Outfit planning",s:"Lay out the week's clothes"},
      ].map(i=><CB key={i.k} label={i.l} sub={i.s} color={T.gold} checked={!!st.weekendChecks?.[i.k]} onChange={()=>set(p=>({...p,weekendChecks:{...p.weekendChecks,[i.k]:!p.weekendChecks?.[i.k]}}))}/>)}
    </Card>
...
function Settings({st,set,tabOrder,setTabOrder}){
  ...
  return <div style={{display:"flex",flexDirection:"column",gap:16}}>
    ...
    <Card><SL icon="🔄" color={T.rose}>Reset</SL><Btn small outline color="#d04040" onClick={()=>set(init())}>Reset All Data</Btn></Card>
...
export default function CatOS(){
  ...
  const[st,_set]=useState(init);
  const[showSettings,setShowSettings]=useState(false);
  const[tabOrder,setTabOrder]=useState(()=>ALL_TABS.map(t=>t.key));
  const set=fn=>_set(p=>typeof fn==="function"?fn(p):fn);
